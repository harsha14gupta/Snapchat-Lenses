...
function renderPassEncodeMesh(targetObject, material, resolution, numberOfRenderTargets) {
    ...
    if (cameraObj) {
        clonedObject.getTransform().setWorldPosition(cameraObj.getTransform().getWorldPosition());
    } else {
        clonedObject.getTransform().setWorldPosition(new vec3(0, 0, 0));
    }
...
function parentCameraSearch(srcObj) {
    var camObj = srcObj;
    if (camObj.getComponent("Component.Camera")) {
        return camObj;
    } else {
        if (camObj.hasParent()) {
            return parentCameraSearch(camObj.getParent());
        } else {
            return null;
        }
    }
